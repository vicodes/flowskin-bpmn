...
export function drawExpandedSubProcess(parentNode, element) {
  ...
  svgAttr(box, { x: 0, y: 0, width, height, rx: 8, ry: 8, fill: T.nodeBg, stroke: T.subBorder, 'stroke-width': 1.5, 'stroke-dasharray': '6 3' });
...
export function drawParticipant(parentNode, element) {
  ...
  const wrapper = embedIcon(parentNode, iconStr, 5, 6, 18);
  svgAppend(parentNode, wrapper);

  // Name — rotated vertically in header
  const name = element.businessObject && element.businessObject.name;
  if (name) {
    ...
    svgAttr(text, {
      ...
      transform: `rotate(180, 15, ${height / 2})`,
    });
...
export function drawGroup(parentNode, element) {
  ...
  const box = svgCreate('rect');
  svgAttr(box, { x: 0, y: 0, width, height, rx: 12, ry: 12, fill: 'none', stroke: T.nodeBorder, 'stroke-width': 1.5, 'stroke-dasharray': '8 4', opacity: '0.6' });
  svgAppend(parentNode, box);
  ...
  if (name) {
    const t = svgCreate('text');
    svgAttr(t, { x: width / 2, y: 16, fill: T.text, 'font-family': "Inter, sans-serif", 'font-size': '10px', 'font-weight': '500', 'text-anchor': 'middle', opacity: '0.7' });
    t.textContent = name;
    svgAppend(parentNode, t);
  }
