...
import { drawActivityMarkers, getActivityMarkers, MARKER_BAND } from './markers.js';
...
// Floor for the label area so a marker row can never squeeze the text to nothing
...
export function drawTask(parentNode, element) {
  ...
  // Label
  const markerBand = getActivityMarkers(element).length ? MARKER_BAND : 0;
  const name = element.businessObject && element.businessObject.name;
  if (name) {
    const padX = 6;
    const textAreaTop = sepY + 2;
    const availW = width - padX * 2;
    const availH = Math.max(MIN_TEXT_HEIGHT, height - textAreaTop - 4 - markerBand);
  ...
  drawActivityMarkers(parentNode, element, width, height);
