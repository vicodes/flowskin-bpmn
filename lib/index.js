...
export function createFlowSkinBPMN(options = {}) {
  ...
  let currentXml = '';
  let hoverCardEnabled = hoverCard !== false;
  let teardownHoverCard = null;

  function stopAllAnims() {
    stopAllEdgeAnimations();
  }

  // Listeners live on the event bus, which outlives an import — attach at most once
  function applyHoverCard() {
    if (hoverCardEnabled && !teardownHoverCard) {
      teardownHoverCard = setupHoverCard(viewer);
    } else if (!hoverCardEnabled && teardownHoverCard) {
      teardownHoverCard();
      teardownHoverCard = null;
    }
  }
  ...
  const api = {
     ...
     * @example
     * beat.setNodeStates({
     *   'Task_1': 'running',    // green spinner
     *   'Task_2': 'completed',  // gray checkmark
     *   'Task_3': 'incident',   // red bolt + pulsating glow
     *   'Task_4': 'hold',       // yellow pause
     * });
     */
    setNodeStates(states) {
      setNodeStates(viewer, states);
    },
    ...
    clearNodeStates(elementIds) {
      clearNodeStates(viewer, elementIds);
    },

    /**
     * Turn the hover details card on or off at runtime.
     * @param {boolean} enabled
     * @returns {boolean} the new state
     */
    setHoverCard(enabled) {
      hoverCardEnabled = enabled !== false;
      applyHoverCard();
      return hoverCardEnabled;
    },

    /**
     * Flip the hover details card on/off.
     * @returns {boolean} the new state
     */
    toggleHoverCard() {
      return api.setHoverCard(!hoverCardEnabled);
    },

    /**
     * Whether the hover details card is currently active.
     * @returns {boolean}
     */
    isHoverCardEnabled() { return hoverCardEnabled; },
...
export { setNodeStates, clearNodeStates } from './badges.js';
export { createFlowSkinModeler } from './modeler.js';
export { flattenBpmn } from './flattenBpmn.js';
